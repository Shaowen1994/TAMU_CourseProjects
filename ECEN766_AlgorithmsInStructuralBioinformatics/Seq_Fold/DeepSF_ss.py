...
from library_ss import load_train_test_data_padding_with_interval,K_max_pooling1d,DLS2F_train_complex_win_filter_layer_opt
...
outputdir = '../Model/Seq_Fold/withSS'

train_datafile=datadir+'/Traindata.list'
val_datafile=datadir+'/validation.list'
test_datafile=datadir+'/Testdata.list'

CV_dir=outputdir+'/withSS_interative_filter'+str(nb_filters)+'_layers'+str(nb_layers)+'_opt'+str(opt)+'_ftsize'+str(filtsize)+'_hn'+str(hidden_num)+'_ktop_node'+str(ktop_node);
